...
export default function Terminal() {
  ...
  useEffect(() => {
    ...
    if (!isVmSupported()) {
      setStatus("error");
      setError(
        "This browser does not support WebAssembly, so the terminal cannot run.",
      );
      return;
    }
    ...
    term.loadAddon(fit);
    term.open(host);
    // Re-fit whenever the host changes size, including when it goes from hidden (0×0) to visible.
    const safeFit = () => {
      ...
      <div className="terminal-bar">
        <span className={"muted small status-" + status}>
          {status === "booting"
            ? "Booting…"
            : status === "ready"
              ? "Ready — BusyBox shell (try ls, cat, grep, vi)"
              : "Error"}
        </span>
